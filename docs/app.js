...
function buildFilters(){
  ...
  $('filters').querySelectorAll('button').forEach(b=>b.onclick=()=>{activeCat=b.dataset.c;if(activeCat==='exhibition')$('genre').value='all';buildFilters();render();});
}
...
function render(){
  ...
  const q=$('q').value.toLocaleLowerCase().trim(),genre=$('genre').value,days=Number($('range').value);
  const end=days?new Date(Date.parse(TODAY)+(days-1)*86400000).toISOString().slice(0,10):null;
  const list=EVENTS.filter(e=>isCat(e,activeCat)&&(genre==='all'||e.genre===genre)&&(!onlySaved||saved.has(key(e)))&&(!end||(!e.start||e.start<=end))&&(!q||[e.title,e.subtitle,e.venue,e.programme,e.genre,e.source,...(e.reasons||[])].join(' ').toLocaleLowerCase().includes(q)));
  const signature=JSON.stringify([activeCat,q,genre,days,onlySaved,sortKey,sortDir]);
...
function load(data){
  ...
  $('sources').innerHTML=(data.sources||[]).map(s=>`<div><strong>${esc(s.name)}</strong> · ${s.status==='ok'?'수집 완료':'갱신 실패'} · ${s.count}개${s.last_success?' · 마지막 성공 '+esc(s.last_success.slice(0,10)):''}${s.status!=='ok'?'<br>원본 목록을 확인하세요. 최근에 확인한 정보가 있으면 최대 7일간 표시합니다.':''}</div>`).join('');
...
['genre','range'].forEach(id=>$(id).addEventListener('change',render));
...
$('reset').onclick=()=>{activeCat='all';onlySaved=false;sortKey='period';sortDir=1;$('q').value='';['genre','range'].forEach(id=>$(id).value='all');$('sort').value='period';$('saved').setAttribute('aria-pressed','false');buildFilters();render();};
